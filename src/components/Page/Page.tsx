...
const PageBtn: FC<btnProps> = memo(
  ({
    handleClick = () => void 0,
    status = false,
    current = false,
...
export default memo<pageProps>(({ total = 1, page, func }) => {
  if (total <= 1) return null;
  return (
    <div className="flex items-center justify-center pt-10 pb-10 space-x-1 flex-wrap">
      <PageBtn handleClick={() => func(page - 1)} status={page === 1}>
        <IconChevronLeft size={12} />
      </PageBtn>
      <DomPosition {...{ total, page, func }} />
      <PageBtn handleClick={() => func(page + 1)} status={page === total}>
