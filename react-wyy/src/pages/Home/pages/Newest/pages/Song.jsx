...
import { top_song } from '@/api'
import { gettopsong } from '@/redux/actions'
export default () => {
    ...
    const handleGetSong = async () => {
        try {
            const { data } = await top_song(type)
            dispatch(gettopsong(data))
        } catch (error) {
            console.log(error);
        }
    }
            ...
            <div className="domhome_newest_song_list">
                {data.map((item, index) => (
                    <div className="item" key={item.id}>
                        ...
                        <span className="name"><div className="text-overflow">{item.name}</div><div>SQ</div></span>
