...
export default memo(({ item, handleClick, handleLetter, isSelf }) => (
  <div className="item w-96 flex p-4 hover:bg-gray-50" key={item.userId}>
    <Link
      to={`/user/${item.userId}`}
      className="relative avatar w-24 h-24 rounded-full border flex-none"
    >
      <img
    ...
    <div className="content flex-auto w-0 pl-2">
      ...
      <div className="info divide-x text-gray-500 mt-1">
        <span className="pr-6">{` 歌单：${item.playlistCount}`}</span>
        <span className="pl-6">{`粉丝：${item.followeds}`}</span>
      </div>
    </div>
  </div>
));
